...
export function createNodeAgentProfile(
  ...
  const context = JSON.stringify({
    objective: snapshot.node.objective, status: snapshot.status,
  ...
  const systemPrompt = [
    ...
    "Work toward the objective and its acceptance criteria using the tools available in this request.",
    ...
    "Do not change Project plans, access another Node's private conversation, or contact another Node. Report coordination needs to the caller.",
  ].join("\n");
