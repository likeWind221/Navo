...
| ✅ | F2.1 通用 Stream RPC MVP 与 Agent 契约 | **共享控制面，由本 Step 唯一写入**；`rpc/**` | 定义一套与框架无关的“请求 → 结果流”RPC 协议：每个请求得到一条响应流，支持取消，两端对数据格式严格校验；第一个业务方法是 `agent.turn`。只做请求对应流式响应，不做 unary、snapshot、重连或多会话并发 |
...
一次 Agent 回合拆成“处理状态 / 过程 / 结果”三层：顶部状态行只显示“处理中 Ns”，进入终态后变为“已处理 Ns”；思考、工具调用与中间说明属于过程，按真实发生顺序逐条展示；最终回答属于结果并始终独立可见。回合结束后过程整体可折叠，折叠边界位于最终回答之前。实施细节见 [F4 过程 UI 实施方案](44-process-ui-design.md)，参考来源见 [过程 UI 开源参考调研](43-devlog-chatgpt-codex-ui-research.md)。
...
| ⬜ | F4.3 过程状态栏、顺序过程流与用时 | 前端 Agent；`frontend/src/workspace/chat/**` | 顶部状态栏在整个生成阶段稳定显示“处理中 {已用时间}”，进入终态后固定为“已处理 {总用时}”，不再根据当前 reasoning 或工具活动动态改标题。状态栏下方按真实事件顺序保留每段过程：活动 reasoning 显示“思考中”，阶段稳定后可显示该段思考摘要；工具显示“执行 {工具名}”（Shell 为“执行 Shell”）；多段 reasoning / 多次工具调用均保留自己的位置。最终回答前提供过程折叠边界，用于折叠 / 展开上方思考与执行过程，最终回答始终独立可见。完成标准：真实模型至少覆盖“思考 → Shell → 思考 → Shell → 最终回答”；顶部只在“处理中 / 已处理”之间切换且计时正确；过程顺序不被覆盖；回合结束后过程自动折叠且可从最终回答前边界重新展开；停止、失败与截断后总用时冻结；窄屏无横向溢出 |
...
当前推进 F4.3 过程状态栏、顺序过程流与用时。F4.3 已取消原“根据当前活动动态推导顶部标题”的设计，改为顶部稳定显示“处理中 Ns / 已处理 Ns”，下方按真实顺序累计“思考中 / 思考摘要、执行 Shell / 其他工具”等过程条目，并在最终回答之前提供折叠边界。F4.2 回合过程折叠已交付，F4.3 在其结构上调整状态栏与折叠交互；见 [F4.2 开发记录](47-devlog-f4-2-process-fold.md) 与 [F4 过程 UI 实施方案](44-process-ui-design.md)。F4.1 自绘图标与“本阶段不引入图标库”的决定见 [F4.1 开发记录](45-devlog-f4-1-icons.md) 与 [图标库选型调研](46-devlog-icon-library-research.md)。F4.4 未开始。
