...
- F9.7b PR 及文档收口提交均已通过 CI；用户随后明确要求接续 F9.7c，等待 F9.7 全部开发完成再合并。
- F9.7c 接续范围与验收见 [66：异常恢复](66-devlog-f9.7c-runtime-recovery.md)；本 Step 不修改 Host、RPC、前端，不实现自动调度、数据库或进程重启恢复。
- 公共 Project 操作协议仍待 F9.9，不把本地类型当作已冻结跨端契约。本轮最终回复交付模块地图、代码机制和验证边界导读；本记录不是导读的替代。
