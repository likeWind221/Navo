...
- 新增 `src/shared/ids.ts`：定义 `SessionId`、`MessageId`、`EventId`、`ToolCallId`、`TurnId`、`StepId` 及构造函数。
...
- canonical message protocol 只使用 `system`、`user`、`assistant` role；工具结果由 `tool-result` 内容块表达，Adapter 再转换为各 Provider 的 wire format。
