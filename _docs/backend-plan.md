...
| ⬜ | F9.6d.2 Resource Lifecycle 与 Access Domain | Project 资源领域 | 把 Resource 收敛为稳定、不可由目录扫描隐式改变的领域对象，通过统一服务管理身份、来源、元数据、内容入口和访问权限，并为 F10 数据库持久化保留稳定接口 | Resource 拥有稳定 ID、Project、name、description、type、source Node、visibility、定向授权、entryRef 与 revision；只能通过 Resource 服务增删改查；默认来源 Node 与 Main 可读，Main 可定向授权或设为 Project 共享；底层在 F9 仍为内存状态 |
| ⬜ | F9.6d.3 Node Resource 与 Main Communication Capability | Node Agent 协作能力 | 让 Node 通过可信 Session Binding 创建正式 Resource、按 Resource ID 读取自己当前可访问的 Resource，并通过单向文本通信把需要进入全局决策的信息发送给 Main | projectId / sourceNodeId 来自 Session Binding；`fetch_resource` 先做 Resource 权限校验再读取受限内容；Node 不能自行授权其他 Node；`send_to_main` 只接受文本且不能指定目标 Node，不自动修改 Roadmap、完成 Node 或启动其他 Agent |
