...
**F9.6f 已正式收口。** GitHub Windows CI 已通过 `pnpm typecheck` 与全量测试（72 test files / 425 tests）。按照当前开发规范，GitHub CI 是 PR 的唯一合并 gate；本机完整测试保留为额外复验，不阻塞合并。 集成验收真实跑通 `Human start Node A -> register_resource -> send_to_main -> Human start Main -> read_mailbox -> set_resource_access -> Node B 保持 idle -> Human start Node B -> Context Builder -> fetch_resource`。验收过程中发现此前 Mailbox 只有 Node 写入 Main 的入口、Main 没有消费入口，因此补入 Main-only `read_mailbox`：它只读当前 Project 的 Node-to-Main append-only 消息，不消费消息、不引入 unread 状态，也不触发 Agent。实现记录见 [81：F9.6f Integration 与 Human Gate](81-devlog-f9.6f-integration-human-gate.md)。F9.6f 与整个 F9.6 均已完成，Phase 9 的下一步唯一进入 F9.7 Human-Controlled ProjectRuntime。**
