...
本机开发环境已补跑完整验证：`pnpm typecheck` 通过；`pnpm test` 全量 54 个测试文件、359 项测试通过；F9.4 定向测试 `tests/project/binding.spec.ts`、`tests/project/session.spec.ts`、`tests/integration/project-agents.spec.ts`、`tests/node/session.spec.ts`、`tests/node/profile-file-read.spec.ts` 共 5 个文件、22 项测试通过。验证覆盖类型契约闭合、Binding 派生与越权拒绝、Main / Node 复用同一 AgentRuntime；未调用真实模型与真实网络。
