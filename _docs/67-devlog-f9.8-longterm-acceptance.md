...
- 2026-09-22 用户在知晓真实验收失败后，明确要求建立 Bug 表并先合并本阶段。PR #30 按该授权收口；真实模型验收仍未通过，F9.8 保留进行中说明，遗留项在 [Bug 跟踪表](bug-plan.md) 的 BUG-001 至 BUG-004 中保持未勾选。文件错误的底层原因和失败 path 尚未留存，“猜测路径”仅为假设，不能视为已确认根因。
- Bug 机制与后续修复笔记规则见 [68：Bug 跟踪机制](68-devlog-bug-tracking.md)。合并不会关闭上述问题，也不替代真实复验。
